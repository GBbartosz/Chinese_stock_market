...
if __name__ == '__main__':
    ...
    for company in companys_dictionary.keys():
        verified_data_result = verify_data(company, companys_dictionary, companys_sector_dic, directory_path, first_date_by_company_dic, first_date_by_sector_collection_dic, min_date, number_of_periods, late_first_dates)
        if verified_data_result is None:
            continue
        else:
            dfs, first_date_by_sector_collection_dic, first_date_by_company_dic, late_first_dates = verified_data_result
    ...
    res = f.create_dictionaries_of_sectors_with_lists(sectors_l, sector_capitalization_dict, sector_pe_ratio_dict, sector_ps_ratio_dict, sector_revenue_dict, sector_revenue_change_dict)
    sector_capitalization_dict, sector_pe_ratio_dict, sector_ps_ratio_dict, sector_revenue_dict, sector_revenue_change_dict = res

    number_of_companies_in_sector = {}
    number_of_companies_in_sector = number_of_companies_in_sector.fromkeys(sectors_l, 0)

    print('number of companies at beginnign: ' + str(len(list(companys_dictionary.keys()))))
    ...
    print(sector_revenue_change_dict)
